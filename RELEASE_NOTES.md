## v1.0.23：v3.2 最终透明风险信号模型

本版本不是把公开数据包装成“最真实”或“最优”概率，而是修正评分中最容易造成误判的重复计权和伪精度，并让导出结果能够复核计算语义。

| 修正 | 实现范围与边界 |
|---|---|
| 独立性优先 | AbuseIPDB 以 `numDistinctUsers` 为主要计数；同一来源的额外报告仅按 25% 弱化计入，避免共享/动态 IP 因同源重复报告被过度放大。 |
| 低置信度抑制 | AbuseIPDB 置信度使用 `confidence^1.3` 的保守变换；低置信度记录不会被固定底座人为抬高。 |
| 同源去重 | ProxyCheck 的无类别 `risk` 在同源 `compromised`、攻击历史或 `scraper` 已命中时不再重复计权。 |
| 量纲修正 | 页面明确区分公开滥用风险 `R / 85`、反向展示分 `P / 100`、透明度 `A / 100` 与证据覆盖度 `C / 100`。 |
| 可回放导出 | JSON 摘要新增模型版本、风险预算、风险主分、透明度、覆盖度、风险桶和逐项证据；不包含授权 Key、IP 历史或浏览器指纹。 |

> v3.2 仍是无真实时间外标签时的可审计产品先验，不能解释为未来恶意概率、账号风险、个人信誉，也不能声称在所有网络环境下最优或最真实。只有经过明确业务定义、时间外标签、分组防泄漏和独立校准集验证后，才能讨论特定业务的校准质量。
