...
纯净度诊断现为 MyIPCheck 独立设计的**公开网络出口风险信号指数**。它不是任何第三方的原始评分，也不是个人、账号、支付或交易的欺诈概率。v3.2 将输出拆为：**公开滥用风险主分、网络透明度、网络上下文和证据覆盖度**。主分只接受可用的公开行为证据；Tor、代理、VPN、中继、IDC、托管、ASN、CIDR、地理与出口差异均被单列解释，绝不凭网络属性推断历史恶意。没有跨服务商、跨时间且独立的真实标签时，不能声称它是普适“最优”或“最真实”评分。

主分中的每一分均由来源、字段、来源质量先验、计数饱和、事件时间衰减和非线性家族组合计算；v3.2 对 AbuseIPDB 以独立报告者为主，额外同源报告只按 25% 弱化计入，并以 `confidence^1.3` 抑制低置信度的伪精度；ProxyCheck 的无类别 risk 在已命中同源具体行为时不再重复计权。APP 同时显示**证据覆盖度**：接口未覆盖、Key 未配置、429、超时、字段缺失或格式错误均不扣风险，但会降低覆盖度，并明确显示为“未覆盖”而不是“未检出”。未经外部、时间外真实标签校准，任何分数都不得解释为概率。

详细的公式、上限、去重方法、覆盖度定义、示例情景和引用见：

- [`purity_scoring_model_v3_1.md`](purity_scoring_model_v3_1.md)（v3.2 最终透明版）
...
应用不会读取账号、Cookie、浏览记录、设备指纹、通讯录或位置权限。IP 查询历史（最多 30 条）、授权凭据、连通性目标、端口目标、安全清单进度和显示偏好均以 Android Keystore AES-GCM 加密后仅保存在本机；检测报告不会上传或创建公开链接。用户主动分享的 Markdown/JSON 摘要不包含授权 Key、加密历史或浏览器指纹数据。网络服务与公开注册表仍可能按照各自隐私政策处理请求日志，使用前请自行确认服务条款、账户配额与隐私政策。
