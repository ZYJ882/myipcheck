# MyIPCheck 公开网络出口风险模型 v3.2（最终透明版）

**状态：** 可解释的公开滥用风险信号指数；不是欺诈概率、个人信誉、账号风控结论或任何第三方的专有评分。它不是“最真实”或普适最优分数，因为没有跨服务商、跨时间且独立的真实标签就无法证明校准准确率。
**版本目标：** 将历史/公开滥用证据与匿名化、基础设施、出口可观测性分离，并将每一个显示分值关联到可查看的来源、字段和时间；对共享/动态 IP 的同源报告重复、低置信标签和字段缺失采取保守处理。
...
v3.2 的每一分都由下列连续函数得到，而不是由模糊的固定档位跳变。每个来源返回的原始记录先映射为带有来源谱系、行为家族、计数、时间和分辨率的证据。
...
| 泛化滥用 | 0.60 | 5 | 60 | AbuseIPDB 置信、独立报告者和弱化后的同源报告量；ipapi.is `is_abuser` 作为独立但较低质量支持。 |
...
v3.2 对 AbuseIPDB 的实现细化为：`n = distinctUsers + floor(0.25 × max(totalReports − distinctUsers, 0))`，并将置信度由 `q` 变换为 `q^1.3`。这样一百条来自同一报告谱系的重复报告不会被视为一百个独立观察；低置信分数也不会因固定底座被人为抬高。`k_g` 是被人工标注为独立的来源谱系数量；不确定谱系一律视为相关，不给予额外交叉加分。85 分的上限保留了不确定性；只有以外部、时间外真实标签得到校准验证后，才可调整预算或声明为概率。
...
应用界面应同时显示 `R / 85` 的风险预算、`P = 100 − R` 的反向展示分、独立透明度 `A`、网络上下文 `X` 和证据覆盖度 `C`。当 `C < 60` 时必须显示“证据不足；未发现不等于无风险”。`P` 不得用于账号封禁、拒绝服务、身份判断或个人信誉判断；它仅适合帮助用户查看本次公开来源中可解释的历史信号。

来源返回 0、无记录、错误、超时、429、字段缺失或过期时，不得奖励 `R`；只能降低 `C` 或显示未覆盖。Tor、VPN、代理、IDC、云、国家、ASN、地理、住宅/数据中心类型和双栈出口差异均不得单独提高 `R`。同一来源的多个字段只保留最强或经过明确弱化的支持；未经独立谱系证明的多源命中不叠加为独立证据。

## 6. 校准与验证路线

此规则是无自有真实标签时可审计、可回放的 v3.2 启发式。若将来合法获得某一明确业务 `b` 中未来 30 天是否发生目标滥用的结果，必须使用评分时点之后的标签、按时间滚动切分并分组防止同 IP/前缀/ASN 泄漏；再以独立校准集评估 PR-AUC、Precision@K、阈值误报率、校准曲线、校准截距/斜率及 Brier 分数。未经这些步骤，APP 不得将 `R`、`P` 或 `A` 表述为发生概率，也不得称其为所有网络环境下的最优或最真实评分。
