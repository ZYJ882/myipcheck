"""MyIPCheck v3.1 公开网络出口风险模型的独立场景验证。
...
def direct_risk(s: Scenario) -> float:
    ...
    support = max(s.abuse_distinct_users, s.abuse_reports)
    abuse = (
        0.60
        * 0.75
        * (0.25 + 0.75 * clamp(s.abuse_score, 0, 100) / 100.0)
        * volume(max(1, support), 5.0)
    ...
    generic = 0.25 * 0.35 * (clamp(s.vendor_risk, 0, 100) / 100.0) ** 1.2 * decay(s.proxy_last_seen_days, 30.0) if s.vendor_risk else 0.0
...
for scenario in [all_clear, missing, abuse_30, abuse_31, old_abuse, proxy_only, tor_only, severe]:
...
print('v3.1 连续性、隔离性、覆盖度与上限断言：通过')
